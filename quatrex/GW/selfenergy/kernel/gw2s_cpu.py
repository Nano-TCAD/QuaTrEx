...
def gw2s_fft_mpi_cpu_3part_sr_bare(
    ...
    Evec = np.concatenate((np.linspace(-(NE-1) * dE, 0.0, NE, endpoint = True, dtype = float), np.linspace(dE, NE*dE, NE, endpoint = True, dtype = float)))
    #Evec = np.concatenate((np.arange(-(NE - 1) * dE, dE, dE), np.arange(dE, (NE + 1) * dE, dE)))
    ...
    gr_principale = linalg_cpu.scalarmul_ifft_cutoff(gr_t_principale, pre_factor/2, ne2, no)[:, ne:]
    ...
    rSigmaR = linalg_cpu.scalarmul_ifft_cutoff(rSigmaR_t, pre_factor/2, ne2, no)[:, ne:]

    sr_principale = rSigmaR + 1j*np.imag(sg-sl)/2 + rSigmaRF
    sr_time = np.real(sr_full[:, ne-1:]) + 1j*np.imag(sg-sl)/2 + rSigmaRF + 1j
    ...
    if(not rank):
        ...
        plt.plot(np.real(gr_principale[0,:]), label = 'principal value real gr')
        plt.legend()
