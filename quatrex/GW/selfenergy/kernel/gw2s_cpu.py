...
def gw2s_fft_mpi_cpu_PI_sr(
    ...
    one_div_by_E = np.concatenate((-1.0/(Evec[-1:0:-1]), np.array([0.0], dtype=np.float64),
                                  1/(Evec[1:]), np.array([1/(Evec[-1] + dE)], dtype=np.float64)))
    one_div_by_E_t = np.fft.fft(one_div_by_E)
...
def gw2s_fft_mpi_cpu_3part_sr_bare(
    ...
    # one_div_by_E[NE-1] = 0
    # one_div_by_E_t = np.fft.fft(one_div_by_E)
    one_div_by_E = np.concatenate((-1.0/(Evec[-1:0:-1]), np.array([0.0], dtype=float),
                                  1/(Evec[1:]), np.array([1/(Evec[-1] + dE)], dtype=float)))
    one_div_by_E_t = np.fft.fft(one_div_by_E)
    ...
    rSigmaR_t = np.multiply(SGmSL_t, one_div_by_E_t)
    # rSigmaR_t = linalg_cpu.elementmul(SGmSL_t, one_div_by_E_t)
    rSigmaR = linalg_cpu.scalarmul_ifft_cutoff(rSigmaR_t, pre_factor, ne2, no)[:, ne:]
    ...
    if (not rank):
        ...
        plt.plot(np.real(gr_new_periodic[0, :]), label='time domain impr. real gr')
        plt.plot(np.real(gr[0, :]), label='reference real gr', linewidth=4, linestyle='--')
        plt.plot(np.real(gr_principale[0, :]), label='principal value real gr', linestyle=':')
        # plt.xlim((5000, 12500))
        plt.ylim((-1, 1))
        plt.legend()
        ...
        plt.plot(np.real(sr[0, :]), label='3 terms real sr')
        plt.plot(np.real((sr_2part_a[0, :]+sr_2part_b[0, :])/2), label='average 2 terms real sr')
        plt.legend()
        plt.savefig('sr_comparison_2.png')

        plt.figure(4, figsize=(20, 10))
        plt.plot(np.imag(sr[0, :]), label='3 terms imag sr')
        plt.plot(np.imag((sr_2part_a[0, :]+sr_2part_b[0, :])/2), label='average 2 terms imag sr')
        plt.legend()
...
@numba.njit("(c16, c16[:,:], c16[:,:], c16[:,:], c16[:,:], c16[:,:], c16[:,:])",
            parallel=True,
            cache=True,
            nogil=True,
            error_model="numpy")
def gw2s_fft_mpi_cpu_3part_sr2(
    pre_factor: np.complex128,
    gg: npt.NDArray[np.complex128],
    gl: npt.NDArray[np.complex128],
    gr: npt.NDArray[np.complex128],
    ...
    wl_t = linalg_cpu.fft_numba(wl, ne2, no)
    wr_t = linalg_cpu.fft_numba(wr, ne2, no)
    # wg_transposed_t = linalg_cpu.fft_numba(-wg.conjugate(), ne2, no)
    # wl_transposed_t = linalg_cpu.fft_numba(-wl.conjugate(), ne2, no)
    ...
    # time reverse
    wr_t_mod = linalg_cpu.reversal(wr_t)
    wl_transposed_t = -linalg_cpu.reversal(wl_t).conjugate()
    wg_transposed_t = -linalg_cpu.reversal(wg_t).conjugate()

    # multiply elementwise the energy reversed with difference of transposed and energy zero
    # see the README for derivation
    wl_transposed = -wl[:, 0].conjugate()
    wg_transposed = -wg[:, 0].conjugate()
    sg_t_2 = linalg_cpu.elementmul(rgg_t, linalg_cpu.substract_special(wl_transposed_t, wl_transposed))
    sl_t_2 = linalg_cpu.elementmul(rgl_t,  linalg_cpu.substract_special(wg_transposed_t, wg_transposed))
    # sr_t_2 = (linalg_cpu.elementmul(rgg_t, np.conjugate(wr_t_mod - np.repeat(wr[:,0].reshape(-1,1), 2*ne, axis=1))) +
    #          linalg_cpu.elementmul(rgr_t, wg_transposed_t - np.repeat(wg[:,0].reshape(-1,1), 2*ne, axis=1)))
    sr_t_2 = (linalg_cpu.elementmul(rgl_t, np.conjugate(linalg_cpu.substract_special(wr_t_mod, wr[:, 0]))) +
              linalg_cpu.elementmul(rgr_t,  linalg_cpu.substract_special(wg_transposed_t, wg_transposed)) +
              linalg_cpu.elementmul(rgr_t, np.conjugate(linalg_cpu.substract_special(wr_t_mod, wr[:, 0]))))
