...
def obc_GF_cpu(M,
    ...
    if not sancho:
        SigRBL[:, :], _, condL, min_dEkL = beyn_func(NCpSC, M[:LBsize, :LBsize].toarray()-SigR[:LBsize, :LBsize].toarray(),
                                                  M[:LBsize, LBsize:2 * LBsize].toarray() - SigR[:LBsize, LBsize:2 * LBsize].toarray(),
    ...
    if np.isnan(condL) or sancho:
        _, SigRBL[:, :], _, condL = open_boundary_conditions(M[:LBsize, :LBsize].toarray() - SigR[:LBsize, :LBsize].toarray(),
                                                       M[LBsize:2 * LBsize, :LBsize].toarray() - SigR[LBsize:2 * LBsize, :LBsize].toarray(),
    ...
    #GR/GL/GG OBC right
    if not sancho:
        SigRBR[:, :], _, condR, min_dEkR = beyn_func(NCpSC, M[NT - RBsize:NT, NT - RBsize:NT].toarray() - SigR[NT - RBsize:NT, NT - RBsize:NT].toarray(),
    ...
    if np.isnan(condR) or sancho:
        _, SigRBR[:, :], _, condR = open_boundary_conditions(M[NT - RBsize:NT, NT - RBsize:NT].toarray() - SigR[NT - RBsize:NT, NT - RBsize:NT].toarray(),
                                                       M[NT - 2 * RBsize:NT - RBsize, NT - RBsize:NT].toarray() - SigR[NT - 2 * RBsize:NT - RBsize, NT - RBsize:NT].toarray(),
                                                       M[NT - RBsize:NT, NT - 2 * RBsize:NT - RBsize].toarray() - SigR[NT - RBsize:NT, NT - 2 * RBsize:NT - RBsize].toarray(),
