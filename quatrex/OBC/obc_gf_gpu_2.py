...
def obc_GF_gpu(M00_left, M01_left, M10_left, M00_right, M01_right, M10_right,
           fL,
           fR,
    ...
    #GR/GL/GG OBC Left
    if not sancho:
        SigRBL_gpu, _, condL, min_dEkL = beyn_func(NCpSC, M00_left, M01_left, M10_left, imag_lim, R, 'L')

    if np.isnan(condL) or sancho:
        _, SigRBL_gpu[:, :], _, condL = open_boundary_conditions(cp.asnumpy(M00_left), cp.asnumpy(M01_left), cp.asnumpy(M10_left), np.eye(LBsize, LBsize))
    ...
    #GR/GL/GG OBC right
    if not sancho:
        SigRBR_gpu, _, condR, min_dEkR = beyn_func(NCpSC, M00_right, M01_right, M10_right, imag_lim, R, 'R')
                                                  
    if np.isnan(condR) or sancho:
        _, SigRBR_gpu[:, :], _, condR = open_boundary_conditions(cp.asnumpy(M00_left), cp.asnumpy(M01_left), cp.asnumpy(M10_left), np.eye(RBsize, RBsize))
    #condR = np.nan
