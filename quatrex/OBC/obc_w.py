...
def obc_w_alt(
        ...
        vh_vec: npt.NDArray[np.complex128],
        mr_vec: npt.NDArray[np.complex128],
        sr_vec: npt.NDArray[np.complex128],
        lg_vec: npt.NDArray[np.complex128],
        ll_vec: npt.NDArray[np.complex128],
        out_flag: np.ndarray,
        bmax: npt.NDArray[np.int32],
        bmin: npt.NDArray[np.int32],
        nbc: int,
    ...
    """
    Apply the OBC fro the screened interaction calculation.
    For multiple energy points. Still bugged->do not use.
    ...
    Args:
        ...
        vh_vec (npt.NDArray[np.complex128]): Effective interaction, vector of sparse matrices
        mr_vec (npt.NDArray[np.complex128]): Helper variable for inversion, vector of sparse matrices
        sr_vec (npt.NDArray[np.complex128]): Helper variable for inversion, vector of sparse matrices
        lg_vec (npt.NDArray[np.complex128]): Helper variable for inversion, vector of sparse matrices
        ll_vec (npt.NDArray[np.complex128]): Helper variable for inversion, vector of sparse matrices
        out_flag (np.ndarray): Flag for every energy point if inversion should be calculated
        bmax (npt.NDArray[np.int32]): End indexes of every block
        bmin (npt.NDArray[np.int32]): Start indexes of every block
        nbc (int): How block size changes after matrix multiplication
        sancho_flag (bool, optional): If sancho OBC should be used, else beyn. Default false
    ...
    for i in range(ne):
        # from G^{\lessgtr}\left(E\right) / G^{r}\left(E\right)
        # (diagonal, upper, lower block of the start block at the left)
        pg_sd, pg_su, pg_sl = dL_OBC_eigenmode.stack_px(pg_vec[i][slb_sd,slb_sd], pg_vec[i][slb_sd,slb_so], nbc)
        pl_sd, pl_su, pl_sl = dL_OBC_eigenmode.stack_px(pl_vec[i][slb_sd,slb_sd], pl_vec[i][slb_sd,slb_so], nbc)
        pr_sd, pr_su, pr_sl = dL_OBC_eigenmode.stack_pr(pr_vec[i][slb_sd,slb_sd], pr_vec[i][slb_sd,slb_so], nbc)
        # (diagonal, upper, lower block of the end block at the right)
        pg_ed, pg_eu, pg_el = dL_OBC_eigenmode.stack_px(pg_vec[i][slb_ed,slb_ed], -pg_vec[i][slb_ed,slb_eo].conjugate().transpose(), nbc)
        pl_ed, pl_eu, pl_el = dL_OBC_eigenmode.stack_px(pl_vec[i][slb_ed,slb_ed], -pl_vec[i][slb_ed,slb_eo].conjugate().transpose(), nbc)
        pr_ed, pr_eu, pr_el = dL_OBC_eigenmode.stack_pr(pr_vec[i][slb_ed,slb_ed], pr_vec[i][slb_ed,slb_eo].transpose(), nbc)

        # from \hat(V)\left(E\right) / G^{r}\left(E\right)
        # (diagonal, upper, lower block of the start block at the left)
        vh_sd, vh_su, vh_sl = dL_OBC_eigenmode.stack_vh(vh_vec[i][slb_sd,slb_sd], vh_vec[i][slb_sd,slb_so], nbc)
        # (diagonal, upper, lower block of the end block at the right)
        vh_ed, vh_eu, vh_el = dL_OBC_eigenmode.stack_vh(vh_vec[i][slb_ed,slb_ed], vh_vec[i][slb_ed,slb_eo].conjugate().transpose(), nbc)

        # from L^{\lessgtr}\left(E\right)
        # (diagonal, upper, lower block of the start block at the left)
        lg_sd, lg_su, _ = dL_OBC_eigenmode.stack_lx(vh_sd, vh_su, vh_sl, pg_sd, pg_su, pg_sl)
        ll_sd, ll_su, _ = dL_OBC_eigenmode.stack_lx(vh_sd, vh_su, vh_sl, pl_sd, pl_su, pl_sl)

        # (diagonal, upper, lower block of the end block at the right)
        lg_ed, _, lg_el = dL_OBC_eigenmode.stack_lx(vh_ed, vh_eu, vh_el, pg_ed, pg_eu, pg_el)
        ll_ed, _, ll_el = dL_OBC_eigenmode.stack_lx(vh_ed, vh_eu, vh_el, pl_ed, pl_eu, pl_el)
        ...
        # correct first and last block to account for the contacts in multiplication
        # first block
        sr_vec[i][slb_sd_mm, slb_sd_mm] = sr_vec[i][slb_sd_mm, slb_sd_mm] + vh_sl @ pr_su
        # last block
        sr_vec[i][slb_ed_mm, slb_ed_mm] = sr_vec[i][slb_ed_mm, slb_ed_mm] + vh_eu @ pr_el

        # calculate M^{r}\left(E\right) (only after above correction)
        mr_vec[i] = sparse.identity(nao, format="csr") - sr_vec[i]


        # correct first and last block to account for the contacts in multiplication
        # first block
        # L^{\lessgtr}_E_00 = V_10*P^{\lessgtr}_E_00*V_01 + V_10*P^{\lessgtr}_E_01*V_00 + V_00*P^{\lessgtr}_E_10*V_01
        lg_vec[i][slb_sd_mm, slb_sd_mm] = lg_vec[i][slb_sd_mm, slb_sd_mm] + vh_sl @ pg_sd @ vh_su
        lg_vec[i][slb_sd_mm, slb_sd_mm] = lg_vec[i][slb_sd_mm, slb_sd_mm] + vh_sl @ pg_su @ vh_sd
        lg_vec[i][slb_sd_mm, slb_sd_mm] = lg_vec[i][slb_sd_mm, slb_sd_mm] + vh_sd @ pg_sl @ vh_su

        ll_vec[i][slb_sd_mm, slb_sd_mm] = ll_vec[i][slb_sd_mm, slb_sd_mm] + vh_sl @ pl_sd @ vh_su
        ll_vec[i][slb_sd_mm, slb_sd_mm] = ll_vec[i][slb_sd_mm, slb_sd_mm] + vh_sl @ pl_su @ vh_sd
        ll_vec[i][slb_sd_mm, slb_sd_mm] = ll_vec[i][slb_sd_mm, slb_sd_mm] + vh_sd @ pl_sl @ vh_su

        # last block
        # L^{\lessgtr}_E_nn = V_nn*PL_E_n-1n*V_nn-1 + V_n-1n*PL_E_nn-1*V_nn + V_n-1n*PL_E_nn*V_nn-1
        lg_vec[i][slb_ed_mm, slb_ed_mm] = lg_vec[i][slb_ed_mm, slb_ed_mm] + vh_ed @ pg_eu @ vh_el
        lg_vec[i][slb_ed_mm, slb_ed_mm] = lg_vec[i][slb_ed_mm, slb_ed_mm] + vh_eu @ pg_el @ vh_ed
        lg_vec[i][slb_ed_mm, slb_ed_mm] = lg_vec[i][slb_ed_mm, slb_ed_mm] + vh_eu @ pg_ed @ vh_el

        ll_vec[i][slb_ed_mm, slb_ed_mm] = ll_vec[i][slb_ed_mm, slb_ed_mm] + vh_ed @ pl_eu @ vh_el
        ll_vec[i][slb_ed_mm, slb_ed_mm] = ll_vec[i][slb_ed_mm, slb_ed_mm] + vh_eu @ pl_el @ vh_ed
        ll_vec[i][slb_ed_mm, slb_ed_mm] = ll_vec[i][slb_ed_mm, slb_ed_mm] + vh_eu @ pl_ed @ vh_el
        ...
        if not np.isnan(cond_l):
            ...
            mr_vec[i][slb_sd_mm,slb_sd_mm] = mr_vec[i][slb_sd_mm,slb_sd_mm] - dmr_sd
            vh_vec[i][slb_sd_mm,slb_sd_mm] = vh_vec[i][slb_sd_mm,slb_sd_mm] - dvh_sd

            if not np.isnan(dll_sd).any():
                lg_vec[i][slb_sd_mm,slb_sd_mm] = lg_vec[i][slb_sd_mm,slb_sd_mm] + dlg_sd
                ll_vec[i][slb_sd_mm,slb_sd_mm] = ll_vec[i][slb_sd_mm,slb_sd_mm] + dll_sd
            else:
                cond_l = np.nan
        ...
        if not np.isnan(cond_r):
            ...
            mr_vec[i][slb_ed_mm,slb_ed_mm]  = mr_vec[i][slb_ed_mm,slb_ed_mm] - dmr_ed
            vh_vec[i][slb_ed_mm,slb_ed_mm]  = vh_vec[i][slb_ed_mm,slb_ed_mm] - dvh_ed

            if not np.isnan(dll_ed).any():
                lg_vec[i][slb_ed_mm,slb_ed_mm] = lg_vec[i][slb_ed_mm,slb_ed_mm] + dlg_ed
                ll_vec[i][slb_ed_mm,slb_ed_mm] = ll_vec[i][slb_ed_mm,slb_ed_mm] + dll_ed
            else:
                cond_r = np.nan

        # set flag if inverse should be computed
        out_flag[i] = not np.isnan(cond_r) and not np.isnan(cond_l)
