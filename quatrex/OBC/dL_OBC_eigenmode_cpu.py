...
def stack_four(a: sparse.csr_matrix, b: sparse.csr_matrix, c: sparse.csr_matrix,
               d: sparse.csr_matrix) -> sparse.csr_matrix:
    """stacks the inputs in the following way:
    [a,b;c,d]
...
def stack_vh(vh_1: sparse.csr_matrix, vh_2: sparse.csr_matrix,
             nbc: np.int64) -> typing.Tuple[sparse.csr_matrix, sparse.csr_matrix, sparse.csr_matrix]:
    ...
    if nbc == 1:
        ...
        vh_d = stack_four(vh_1, vh_2, vh_2_ct, vh_1)
        vh_u = stack_three(zs_2, vh_2, zs_1)
        vh_l = vh_u.conjugate().transpose()
        ...
        vh_d = stack_nine(vh_1, vh_2, zs_1, vh_2_ct, vh_1, vh_2, zs_1, vh_2_ct, vh_1)
        vh_u = stack_three(zs_3, vh_2, zs_2)
        vh_l = vh_u.conjugate().transpose()
...
def stack_pr(pr_1: sparse.csr_matrix, pr_2: sparse.csr_matrix,
             nbc: np.int64) -> typing.Tuple[sparse.csr_matrix, sparse.csr_matrix, sparse.csr_matrix]:
    ...
    if nbc == 1:
        ...
        zs_1 = sparse.csr_array((lb, lb), dtype=np.complex128)
        zs_2 = sparse.csr_array((lb, 2 * lb), dtype=np.complex128)
        pr_d = stack_four(pr_1, pr_2, pr_2_t, pr_1)
        pr_u = stack_three(zs_2, pr_2, zs_1)
        pr_l = pr_u.transpose()
        ...
        zs_3 = sparse.csr_array((2 * lb, 3 * lb), dtype=np.complex128)
        # diagonal, upper, lower blocks
        pr_d = stack_nine(pr_1, pr_2, zs_1, pr_2_t, pr_1, pr_2, zs_1, pr_2_t, pr_1)
        pr_u = stack_three(zs_3, pr_2, zs_2)
        pr_l = pr_u.transpose()
    else:
        raise ValueError("Argument error, type input not possible")
...
def stack_px(px_1: sparse.csr_matrix, px_2: sparse.csr_matrix,
             nbc: np.int64) -> typing.Tuple[sparse.csr_matrix, sparse.csr_matrix, sparse.csr_matrix]:
    ...
    if nbc == 1:
        ...
        zs_1 = sparse.csr_array((lb, lb), dtype=np.complex128)
        zs_2 = sparse.csr_array((lb, 2 * lb), dtype=np.complex128)
        px_d = stack_four(px_1, px_2, -px_2_ct, px_1)
        px_u = stack_three(zs_2, px_2, zs_1)
        px_l = -px_u.conjugate().transpose()
        ...
        zs_3 = sparse.csr_array((2 * lb, 3 * lb), dtype=np.complex128)
        # diagonal, upper, lower blocks
        px_d = stack_nine(px_1, px_2, zs_1, -px_2_ct, px_1, px_2, zs_1, -px_2_ct, px_1)
        px_u = stack_three(zs_3, px_2, zs_2)
        px_l = -px_u.conjugate().transpose()
    else:
        raise ValueError("Argument error, type input not possible")
...
def stack_mr(vh_d: sparse.csr_matrix, vh_u: sparse.csr_matrix, vh_l: sparse.csr_matrix, pr_d: sparse.csr_matrix,
             pr_u: sparse.csr_matrix,
             pr_l: sparse.csr_matrix) -> typing.Tuple[sparse.csr_matrix, sparse.csr_matrix, sparse.csr_matrix]:
...
def stack_lx(vh_d: sparse.csr_matrix, vh_u: sparse.csr_matrix, vh_l: sparse.csr_matrix, px_d: sparse.csr_matrix,
             px_u: sparse.csr_matrix,
             px_l: sparse.csr_matrix) -> typing.Tuple[sparse.csr_matrix, sparse.csr_matrix, sparse.csr_matrix]:
    ...
    """
    lx_d = (vh_l @ px_d @ vh_u + vh_l @ px_u @ vh_d + vh_d @ px_l @ vh_u + vh_d @ px_d @ vh_d + vh_d @ px_u @ vh_l +
            vh_u @ px_l @ vh_d + vh_u @ px_d @ vh_l)

    lx_u = (vh_l @ px_u @ vh_u + vh_d @ px_d @ vh_u + vh_d @ px_u @ vh_d + vh_u @ px_l @ vh_u + vh_u @ px_d @ vh_d)
...
def get_mm_obc(
    vh_1: sparse.csr_matrix, vh_2: sparse.csr_matrix, pg_1: sparse.csr_matrix, pg_2: sparse.csr_matrix,
    pl_1: sparse.csr_matrix, pl_2: sparse.csr_matrix, pr_1: sparse.csr_matrix, pr_2: sparse.csr_matrix, nbc: int
                  ...
                  typing.Tuple[npt.NDArray[np.complex128], npt.NDArray[np.complex128], npt.NDArray[np.complex128]],
                  typing.Tuple[npt.NDArray[np.complex128], npt.NDArray[np.complex128]], typing.Tuple[
                      npt.NDArray[np.complex128], npt.NDArray[np.complex128]], typing.Tuple[
                          npt.NDArray[np.complex128], npt.NDArray[np.complex128]], typing.Tuple[
    ...
    if nbc == 1:
        ...
        lg_d2[:, :] = ((vh_l1 @ pg_d1 @ vh_u1).toarray() + (vh_l1 @ pg_u1 @ vh_d1).toarray() +
                       (vh_d1 @ pg_l1 @ vh_u1).toarray() + (vh_d1 @ pg_d1 @ vh_d1).toarray() +
                       (vh_d1 @ pg_u1 @ vh_l1).toarray() + (vh_u1 @ pg_l1 @ vh_d1).toarray() +
                       (vh_u1 @ pg_d1 @ vh_l1).toarray())
        lg_u2[:, :] = ((vh_l1 @ pg_u1 @ vh_u1).toarray() + (vh_d1 @ pg_d1 @ vh_u1).toarray() +
                       (vh_d1 @ pg_u1 @ vh_d1).toarray() + (vh_u1 @ pg_l1 @ vh_u1).toarray() +
                       (vh_u1 @ pg_d1 @ vh_d1).toarray())
        ll_d2[:, :] = ((vh_l1 @ pl_d1 @ vh_u1).toarray() + (vh_l1 @ pl_u1 @ vh_d1).toarray() +
                       (vh_d1 @ pl_l1 @ vh_u1).toarray() + (vh_d1 @ pl_d1 @ vh_d1).toarray() +
                       (vh_d1 @ pl_u1 @ vh_l1).toarray() + (vh_u1 @ pl_l1 @ vh_d1).toarray() +
                       (vh_u1 @ pl_d1 @ vh_l1).toarray())
        ll_u2[:, :] = ((vh_l1 @ pl_u1 @ vh_u1).toarray() + (vh_d1 @ pl_d1 @ vh_u1).toarray() +
                       (vh_d1 @ pl_u1 @ vh_d1).toarray() + (vh_u1 @ pl_l1 @ vh_u1).toarray() +
                       (vh_u1 @ pl_d1 @ vh_d1).toarray())
        dmr_lu[:, :] = -(vh_l1 @ pr_u1).toarray()
        dmr_ul[:, :] = -(vh_u1 @ pr_l1).toarray()
        dlg_lu[:, :] = (vh_l1 @ pg_d1 @ vh_u1).toarray() + (vh_l1 @ pg_u1 @ vh_d1).toarray() + (
            vh_d1 @ pg_l1 @ vh_u1).toarray()
        dll_lu[:, :] = (vh_l1 @ pl_d1 @ vh_u1).toarray() + (vh_l1 @ pl_u1 @ vh_d1).toarray() + (
            vh_d1 @ pl_l1 @ vh_u1).toarray()
        dlg_ul[:, :] = (vh_u1 @ pg_d1 @ vh_l1).toarray() + (vh_u1 @ pg_l1 @ vh_d1).toarray() + (
            vh_d1 @ pg_u1 @ vh_l1).toarray()
        dll_ul[:, :] = (vh_u1 @ pl_d1 @ vh_l1).toarray() + (vh_u1 @ pl_l1 @ vh_d1).toarray() + (
...
def get_dl_obc_start(mr_x: np.ndarray, xr_d: np.ndarray, xr_d_ct: np.ndarray, lx_d: np.ndarray,
                     lx_o: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray]:
...
def get_dl_obc_end(eivec: np.ndarray, eivec_ct: np.ndarray, ieivec: np.ndarray, ieivec_ct: np.ndarray,
                   eival_sq: np.ndarray, sl_x: slice, fx: np.ndarray, mr_x: np.ndarray, mr_x_ct: np.ndarray,
                   xr_d: np.ndarray, xr_d_ct: np.ndarray, ax_diff: np.ndarray, ref_iteration: int) -> np.ndarray:
    """Helper function to replace symmetric computations 
    for both greater and lesser obc corrections.
...
def get_dl_obc(xr_d: np.ndarray, lg_d: np.ndarray, lg_o: np.ndarray, ll_d: np.ndarray, ll_o: np.ndarray,
               mr_x: np.ndarray, blk: str) -> typing.Tuple[np.ndarray, np.ndarray]:
    ...
    ag_diff, fg = get_dl_obc_start(mr_x, xr_d, xr_d_ct, lg_d, lg_o)
    al_diff, fl = get_dl_obc_start(mr_x, xr_d, xr_d_ct, ll_d, ll_o)
    ...
    dlg_d = get_dl_obc_end(eivec, eivec_ct, ieivec, ieivec_ct, eival_sq, sl_x, fg, mr_x, mr_x_ct, xr_d, xr_d_ct,
                           ag_diff, ref_iteration)
    ...
    dll_d = get_dl_obc_end(eivec, eivec_ct, ieivec, ieivec_ct, eival_sq, sl_x, fl, mr_x, mr_x_ct, xr_d, xr_d_ct,
                           al_diff, ref_iteration)

    return dlg_d, dll_d


def get_dl_obc_alt(xr_d: np.ndarray, lg_d: np.ndarray, lg_o: np.ndarray, ll_d: np.ndarray, ll_o: np.ndarray,
                   mr_x: np.ndarray, blk: str) -> typing.Tuple[np.ndarray, np.ndarray]:
    """
    Calculates open boundary corrections for lg and ll.
