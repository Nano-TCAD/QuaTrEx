...
# TODO: Refactor to a proper unit test
if __name__ == "__main__":

    rng = np.random.default_rng(42)

    folder = "/home/alziogas/Projects/quatrex_matlab"
    inputs = dict()
    ...
    for factor in (1, 2, 4):
        for type in ('L', 'R'):
            ...
            N00, N01, N10, blocks = extract_small_matrix_blocks(inputs['M00'], inputs['M01'], inputs['M10'], factor, type)
            
            outputs = dict()
            for name in ('M00', 'M01', 'M10'):
                filename = os.path.join(folder, f'out_{name}_{factor}_{type}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[f'N{name[1:]}']
            filename = os.path.join(folder, f'out_blocks_{factor}_{type}.mat')
            mat = io.loadmat(filename)
            ref_blocks = mat['blocks'][0]
            ...
            LP0, LP1, RP0, RP1 = contour_integral(factor, blocks, N00.shape[0], 1.0, type, YL=YL, YR=YR)
            for name in ('LP0', 'LP1', 'RP0', 'RP1'):
                filename = os.path.join(folder, f'out_{name}_{factor}_{type}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[name]
            assert np.allclose(LP0, outputs['LP0'])
            assert np.allclose(LP1, outputs['LP1'])
            assert np.allclose(RP0, outputs['RP0'])
            assert np.allclose(RP1, outputs['RP1'])

            LV, LS, LW, RV, RS, RW = beyn_svd(LP0, RP0, eps_lim=1e-8)
            for name in ("LV", "LS", "LW", "RV", "RS", "RW"):
                filename = os.path.join(folder, f'out_{name}_{factor}_{type}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[name]
            # assert np.allclose(LV, outputs['LV'])
            # assert np.allclose(LS, outputs['LS'])
            # assert np.allclose(LW, outputs['LW'])
            # assert np.allclose(RV, outputs['RV'])
            # assert np.allclose(RS, outputs['RS'])
            # assert np.allclose(RW, outputs['RW'])
            assert np.allclose(LV @ LS @ LW.conj().T, outputs['LV'] @ outputs['LS'] @ outputs['LW'].conj().T)
            assert np.allclose(RV @ RS @ RW.conj().T, outputs['RV'] @ outputs['RS'] @ outputs['RW'].conj().T)
            # assert np.allclose(LV.conj().T @ LP1 @ LW @ np.linalg.inv(LS),
            #                    outputs['LV'].conj().T @ outputs['LP1'] @ outputs['LW'] @ np.linalg.inv(outputs['LS']))
            # assert np.allclose(np.linalg.inv(RS) @ RV.conj().T @ RP1 @ RW,
            #                     np.linalg.inv(outputs['RS']) @ outputs['RV'].conj().T @ outputs['RP1'] @ outputs['RW'])

            Lu, Llambda, Ru, Rlambda = beyn_eig(LV, LS, LW, LP1, RV, RS, RW, RP1)
            for name in ("Lu", "Llambda", "Ru", "Rlambda"):
                filename = os.path.join(folder, f'out_{name}_{factor}_{type}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[name]
            # assert np.allclose(Lu, outputs['Lu'])
            assert np.allclose(Llambda, outputs['Llambda'].reshape(-1))
            # assert np.allclose(Ru, outputs['Ru'])
            assert np.allclose(Rlambda, outputs['Rlambda'].reshape(-1))

            kL, kR, phiL, phiR = beyn_phi(LV, Lu, Llambda, RW, Ru, Rlambda, factor, type)
            for name in ("kL", "kR", "phiL", "phiR"):
                filename = os.path.join(folder, f'out_{name}_{factor}_{type}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[name]
            assert np.allclose(kL, outputs['kL'].reshape(-1))
            assert np.allclose(kR, outputs['kR'].reshape(-1))
            # assert np.allclose(phiL, outputs['phiL'])
            assert np.allclose(phiR, outputs['phiR'])

            Sigma, gR, min_dEk = beyn_sigma(kL, kR, phiL, phiR, N00, N01, N10, 0.5, 2, type)
            ...
            # ksurfL, VsurfL, inv_VsurfL, dEk_dk = prepare_input_data(kL, kR, phiL, phiR, N01, N10, 0.5, 1.0)
            # for name in ("ksurfL", "VsurfL", "inv_VsurfL", "dEk_dk"):
            #     filename = os.path.join(folder, f'out_{name}_{factor}.mat')
            #     mat = io.loadmat(filename)
            #     outputs[name] = mat[name]
            # assert np.allclose(ksurfL, outputs['ksurfL'].reshape(-1))
            # assert np.allclose(VsurfL, outputs['VsurfL'])
            # assert np.allclose(inv_VsurfL, outputs['inv_VsurfL'])
            # assert np.allclose(dEk_dk, outputs['dEk_dk'])

            # # gR = np.linalg.inv(N00 + N10 @ VsurfL @ np.diag(np.exp(-1j * ksurfL)) @ inv_VsurfL)
            # gR = N10 @ VsurfL @ np.diag(np.exp(-1j * ksurfL)) @ inv_VsurfL
            # ind = np.where(abs(dEk_dk))
            # if len(ind[0]) > 0:
            #     min_dEk = np.min(abs(dEk_dk[ind]))
            # else:
            #     min_dEk = 1e8
            # for name in ("gR", "min_dEk"):
            #     filename = os.path.join(folder, f'out_{name}_{factor}.mat')
            #     mat = io.loadmat(filename)
            #     outputs[name] = mat[name]
            # assert np.allclose(gR, outputs['gR'])
            ...
            # Sigma, gR, cond, min_dEk = beyn(factor, blocks, N00, N01, N10, 0.5, 1.0, 'L', YL=YL, YR=YR)
            # LP0, LP1, RP0, RP1, LV, LS, LW, RV, RS, RW, Lind, Rind = beyn(factor, blocks, N00, N01, N10, 0.5, 1.0, 'L', Y=Y)
            # LV, LS, LW, Llambda, phiL, RV, RS, RW, Rlambda, phiR = beyn(factor, blocks, N00, N01, N10, 0.5, 1.0, 'L', Y=Y)

            # outputs = dict()
            # for name in ("LV", "LS", "LW", 'Llambda', 'phiL', "RV", "RS", "RW", "Rlambda", "phiR"):
            #     filename = os.path.join(folder, f'out_{name}_{factor}.mat')
            #     mat = io.loadmat(filename)
            #     outputs[name] = mat[name]
            # assert np.allclose(LV, outputs['LV'])
            # assert np.allclose(np.diag(LS), outputs['LS'])
            # assert np.allclose(LW, outputs['LW'])
            # assert np.allclose(Llambda, outputs['Llambda'].reshape(-1))
            # assert np.allclose(phiL, outputs['phiL'])
            # # assert np.allclose(RV, outputs['RV'])
            # assert np.allclose(np.diag(RS), outputs['RS'])
            # # assert np.allclose(RW, outputs['RW'])
            # assert np.allclose(Rlambda, outputs['Rlambda'].reshape(-1))
            # print(phiR)
            # print(outputs['phiR'])
            # print()
            # print(phiR - outputs['phiR'])
            # print()
            # assert np.allclose(phiR, outputs['phiR'])

            outputs = dict()
            for name in ('Sigma', 'gR', 'cond', 'min_dEk'):
                filename = os.path.join(folder, f'out_{name}_{factor}.mat')
                mat = io.loadmat(filename)
                outputs[name] = mat[name]
            print(Sigma)
            ...
            assert np.allclose(Sigma, outputs['Sigma'])
            assert np.allclose(gR, outputs['gR'])
            assert np.allclose(cond, outputs['cond'])
            # assert np.allclose(min_dEk, outputs['min_dEk'])
    exit(0)


    N = 100
    M00 = rng.random((N, N)) + 1j * rng.random((N, N))
    M01 = rng.random((N, N)) + 1j * rng.random((N, N))
    M10 = rng.random((N, N)) + 1j * rng.random((N, N))

    for factor in (1, 2, 3, 4):
        m00, m01, m10, matrix_blocks = extract_small_matrix_blocks(M00, M01, M10, factor, 'L')
        print(f'factor = {factor}: {len(matrix_blocks)} blocks of size:')
        # for key, value in matrix_blocks.items():
        #     print(f'  {key}: {value.shape}')
        for i in range(len(matrix_blocks)):
            print(f'  {i}: {matrix_blocks[i].shape}')
