...
if __name__ == "__main__":
    ...
    # solution_path = os.path.join(scratch_path, "for_kevinfixed")
    solution_path = os.path.join(scratch_path, "32_kevin")
    ...
    # energy = np.linspace(-6, -2, 1000, endpoint=True, dtype=float)  # Energy Vector
    energy = np.linspace(-20, 6, 6500, endpoint=True, dtype=float)  # Energy Vector
    ...
    # vharray = vh.toarray()
    # np.savetxt(parent_path + 'oldcoulomb.dat', vharray)
    if args.bsr:
    ...
    for iter_num in range(max_iter):
        ...
        if iter_num == 2 and epsR < 5:
            guessleft = ECmin_vec[iter_num] - 0.2
            guessright = ECminright_vec[iter_num] - 0.2
