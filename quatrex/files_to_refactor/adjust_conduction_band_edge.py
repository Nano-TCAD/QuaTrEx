...
def calc_bandstructure_interpol(
    ...
    SigL1 = 1j * np.imag(Self_energy_lesser[index_energy_close_target])
    SigL2 = 1j * np.imag(Self_energy_lesser[index_energy_close_target + 1])
    SigL = SigL1 + (SigL2 - SigL1) / (energy_higher_target - energy_lower_target) * (target_energy - energy_lower_target)
    SigL = (SigL - SigL.conj().T) / 2

    SigG1 = 1j * np.imag(Self_energy_greater[index_energy_close_target])
    SigG2 = 1j * np.imag(Self_energy_greater[index_energy_close_target + 1])
    SigG = SigG1 + (SigG2 - SigG1) / (energy_higher_target - energy_lower_target) * (target_energy - energy_lower_target)
    SigG = (SigG - SigG.conj().T) / 2

    SigR1 = Self_energy_retarded[index_energy_close_target]
    SigR2 = Self_energy_retarded[index_energy_close_target + 1]
    SigR = SigR1 + (SigR2 - SigR1) / (energy_higher_target - energy_lower_target) * (target_energy - energy_lower_target)
