...
def rgf_GF(M, SigL, SigG, GR, GRnn1, GL, GLnn1, GG, GGnn1, DOS, fL, fR, Bmin_fi, Bmax_fi, factor = 1.0, index_E = 0, sancho = False, min_dEk = 1e8):
    ...
    if np.isnan(condR) or sancho:
        _, SigRBR, _, condR = open_boundary_conditions(M[NT - RBsize:NT, NT - RBsize:NT].toarray(),
                                                      M[NT - 2*RBsize:NT - RBsize, NT - RBsize:NT].toarray(),
                                                        M[NT - RBsize:NT, NT - 2*RBsize:NT - RBsize].toarray(), np.eye(RBsize, RBsize))
    #condR = np.nan
    ...
    gL[-1, 0:NN, 0:NN] = gR[-1, 0:NN, 0:NN] @ SigL[Bmin[-1]:Bmax[-1]+1, Bmin[-1]:Bmax[-1]+1].toarray() @ gR[-1, 0:NN, 0:NN].T.conj()
    gG[-1, 0:NN, 0:NN] = gR[-1, 0:NN, 0:NN] @ SigG[Bmin[-1]:Bmax[-1]+1, Bmin[-1]:Bmax[-1]+1].toarray() @ gR[-1, 0:NN, 0:NN].T.conj()
...
if __name__ == '__main__':
    ...
    SBR = (SBR+SBR.T)/2
    SB = sparse.block_diag(
        (SBL, sparse.csc_matrix((n*(nBlocks-2), n*(nBlocks-2))), SBR))
    M = H+SB
    ...
    rgf_GF(M.copy(), SL.copy(), SG.copy(), GR_3D_E[0], GRnn1_3D_E[0], GL_3D_E[0], GLnn1_3D_E[0], GG_3D_E[0], GGnn1_3D_E[0], 0.5, 0.5, block_starts.copy()+1, block_ends.copy()+1, sancho=True)
    ...
    GL_2D = mat_assembly_fullG(GL_3D_E[0], GLnn1_3D_E[0], block_starts.copy() + 1, block_ends.copy() + 1, format='sparse', type = 'L')
    GG_2D = mat_assembly_fullG(GG_3D_E[0], GGnn1_3D_E[0], block_starts.copy() + 1, block_ends.copy() + 1, format='sparse', type = 'G')
    GR_2D = mat_assembly_fullG(GR_3D_E[0], GRnn1_3D_E[0], block_starts.copy() + 1, block_ends.copy() + 1, format='sparse', type = 'R')
