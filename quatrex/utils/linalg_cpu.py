...
@numba.njit("(c16[:,:],c16[:])", parallel=True, cache=True, nogil=True, error_model="numpy")
...
@numba.njit("(c16[:,:], i4[:])", parallel=True, cache=True, nogil=True, error_model="numpy")
...
@numba.njit("(c16[:,:], c16)", parallel=True, cache=True, nogil=True, error_model="numpy")
def scalarmul(g1: npt.NDArray[np.complex128], fac: complex) -> npt.NDArray[np.complex128]:
...
@numba.njit("(c16[:,:], c16[:,:])", parallel=True, cache=True, nogil=True, error_model="numpy")
def elementmul(g1: npt.NDArray[np.complex128], g2: npt.NDArray[np.complex128]) -> npt.NDArray[np.complex128]:
...
@numba.njit("(c16[:,:], i8, i8)", parallel=True, cache=True, nogil=True, error_model="numpy")
...
@numba.njit("(c16[:,:], c16, i8, i8)", parallel=True, cache=True, nogil=True, error_model="numpy")
def scalarmul_ifft(g1: npt.NDArray[np.complex128], fac: np.complex128, ne: np.int64,
                   no: np.int64) -> npt.NDArray[np.complex128]:
    """IFFT, crops, and multiplies elementwise g1 with fac
        Needs RocketFFT! https://github.com/styfenschaer/rocket-fft
        Numba scalarmul and fft separately takes more or less the same time
...
@numba.njit("(i4, i4, c16[:,:], c16[:,:])", parallel=True, cache=True, nogil=True, error_model="numpy")
def elementmult_chunk(start: np.int32, end: np.int32, g1: npt.NDArray[np.complex128],
...
def correlate_3D(a: np.ndarray, b: np.ndarray, mode="full", b_index="ji", method="fft", n_worker=32):
    '''
...
def convolve_3D(a: np.ndarray, b: np.ndarray, mode="valid", b_index="ij", method="fft", n_worker=32):
    '''
