...
if __name__ == "__main__":
    ...
    for ne in nes:
        ...
        for lb in lbs:
            ...
            dDOSm_gold_global = np.concatenate(([0], np.max(np.abs(dos_global[1:ne-1, :] / (dos_global[0:ne-2, :] + 1)), axis=1), [np.max(np.abs(dos_global[ne-1,:]/(dos_global[ne-2,:] + 1)))]))
            dDOSp_gold_global = np.concatenate(([np.max(np.abs(dos_global[0,:]/(dos_global[1,:] + 1)))], np.max(np.abs(dos_global[1:ne-1, :] / (dos_global[2:ne, :] + 1)), axis=1), [0]))
            ...
            if size == 1:
                dDOSm = np.concatenate(([0], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[0:ne_loc-2, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(dos_loc[ne_loc-2,:] + 1)))]))
                dDOSp = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(dos_loc[1,:] + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[2:ne_loc, :] + 1)), axis=1), [0]))
            elif rank == 0:
                dDOSm = np.concatenate(([0], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[0:ne_loc-2, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(dos_loc[ne_loc-2,:] + 1)))]))
                dDOSp = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(dos_loc[1,:] + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[2:ne_loc, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(buf_recv_r + 1)))]))
            elif rank == size - 1:
                dDOSm = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(buf_recv_l + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[0:ne_loc-2, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(dos_loc[ne_loc-2,:] + 1)))]))
                dDOSp = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(dos_loc[1,:] + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[2:ne_loc, :] + 1)), axis=1), [0]))
            else:
                dDOSm = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(buf_recv_l + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[0:ne_loc-2, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(dos_loc[ne_loc-2,:] + 1)))]))
                dDOSp = np.concatenate(([np.max(np.abs(dos_loc[0,:]/(dos_loc[1,:] + 1)))], np.max(np.abs(dos_loc[1:ne_loc-1, :] / (dos_loc[2:ne_loc, :] + 1)), axis=1), [np.max(np.abs(dos_loc[ne_loc-1,:]/(buf_recv_r + 1)))]))
