"""This file is based on/copied from the GPAW hilbert transform implementation. Which is based on the paper: 
    ...
    For the Hilbert transform that we want to compute, the setting is as follows:
        timeordered=False,
...
class HilbertTransform:
    def __init__(self, omega_w, eta=1e-6, quatrex=False, ret=True, timeordered=False, gw=False,
                 blocksize=500):
        ...
        -----------
        omega_w: array
            Frequencies (energies) at which the Hilbert transform is evaluated.
        eta: float
            Broadening factor. Small complex part (should have default value).
        ret: bool
            If True, return the retarded part of the Hilbert transform (False for advanced part). Only for testing
        """
    ...
    def __call__(self, S_wx):
        """Inplace transform
        For quatrex, the S_wx is a 2D array with shape (nw, nx) where nw is the number of frequencies
        and nx is the number of orbitals.
        Blocksize is """
        B_wx = S_wx.reshape((len(S_wx), -1))
        tmp_wx = self.H_ww @ B_wx
        return tmp_wx.reshape(S_wx.shape)
...
class GWHilbertTransforms:
    ...
    def __call__(self, A_wGG):
        ...
        tmp_xy = H_xw @ A_wy  # np.dot(H_xw, A_wy)
        return tmp_xy.reshape((2, *A_wGG.shape))  # what about this reshape?
