...
class Matrices:
    ...
    rows_cm: npt.NDArray = None     # Coulomb matrix
    columns_cm: npt.NDArray = None  # Coulomb matrix
    ...
    def __init__(self, sim_folder, Nk=np.array([1,1,1]), kp_shift=np.array([0.0, 0.0, 0.0]), Vappl=0.0, bias_point=0, potential_type='linear', rank=0):
        """
        Parameters
        ----------
        sim_folder : str
        ...
        if not rank:
            ...
            # Check if the Coulomb matrix is symmetric
            if not self.is_symmetric(self.Coulomb_matrix):
                print("Coulomb matrix is not symmetric. Symmetrizing...")
            # Symmetrice the Coulomb matrix
            self.symmetrize_coulomb_matrix()
            ...
            for key in self.Hamiltonian.keys():
                if key == (0, 0, 0):
                    self.Overlap[key] = sparse.identity(self.size, format='csr', dtype=np.complex128)
                else:
            ...
            assert self.NBlock != 3, f"Number of blocks is 3. NBlock: {self.NBlock}. This can cause some problems because the calculation of the screened interaction is then only for a single block."
    ...
    def prepare_block_properties(self, matrix):
        ...
        NBlock = len(Bmin)  # or len(Bmax)
        return NBlock, Bmin, Bmax
    ...
    def map_sparse_indices(self, k_Matrix):
        ...
        """
        rows, columns = _nz_indices(k_Matrix)
        return rows, columns
    ...
    def symmetrize_coulomb_matrix(self):
        """
        Symmetrizes the Coulomb matrix.
        """
        added_keys = []
        for key in self.Coulomb_matrix.keys():
            if key in added_keys:
                continue
            inv_key = (-key[0], -key[1], -key[2])
            self.Coulomb_matrix[key] = (self.Coulomb_matrix[key] + self.Coulomb_matrix[inv_key].T)/2
            self.Coulomb_matrix[inv_key] = self.Coulomb_matrix[key].T
            added_keys.append(inv_key)
    ...
    def is_symmetric(self, matrix):
        ...
        for key in matrix.keys():
            inv_key = (-key[0], -key[1], -key[2])
            if not np.allclose(matrix[key].todense(), matrix[inv_key].todense().T):
                return False
        return True
