...
class Hamiltonian:
    ...
    def homogenize(self, NCpSC):
        ...
        self.Overlap['H_4'] = homogenize_matrix_Rnosym(S00, S01, S10, len(self.Bmin))# Copyright 2023 ETH Zurich and the QuaTrEx authors. All rights reserved."""
...
class Hamiltonian:
    ...
    def __init__(self, sim_folder, no_orb, num_kpoints=np.array([1,1,1]), Vappl=0.0, bias_point=0, potential_type='linear', layer_matrix='/Layer_Matrix.dat', homogenize=False, NCpSC=1, rank=0):
        if (not rank):
            self.no_orb = no_orb
            self.sim_folder = sim_folder
            self.blocks = glob.glob(self.sim_folder + '/H*.bin')
            ...
            self.keys = [i.rsplit('/')[-1].rsplit('.bin')[0] for i in self.blocks]
            if num_kpoints[0]-1 > 0:
                raise ValueError('Warning: k-points in transport direction')
            if num_kpoints[1]-1 > 0 and ('H_1' not in self.keys or 'H_7' not in self.keys):
                raise ValueError('Warning: k-points in y-direction and H_1 or H_7 not found')
            if num_kpoints[2]-1 > 0 and ('H_3' not in self.keys or 'H_5' not in self.keys):
                raise ValueError('Warning: k-points in z-direction and H_3 or H_5 not found')
            ...
            if not self.Sblocks:
                for ii in range(0, len(self.blocks)):
                    ...
                    if self.keys[ii] == 'H_4':
                        ...
                        self.Overlap[self.keys[ii]] = sparse.csr_matrix((self.NH, self.NH), dtype=np.cfloat)
            ...
            #Read Block Properties
            self.LM = read_file_to_float_ndarray(sim_folder + layer_matrix)
            self.lattice_vectors, _ = read_lattice_dat(sim_folder)
            self.NA = self.LM.shape[0]
            self.NB = self.LM.shape[1] - 4
            self.TB = np.max(self.no_orb)
            self.Smin = read_file_to_int_ndarray(sim_folder + '/Smin_dat')
            ...
            # Set kpoints
            self.kp = self.k_points(num_kpoints)
            # Create k-dependent Hamiltonian and Coulomb matrices
            self.Coulomb_matrix = self.construct_coulomb_matrix()
            self.k_Hamiltonian = self.create_k_matrix()
            self.k_Coulomb_matrix = self.create_k_matrix(int_mat=self.Coulomb_matrix)
            self.k_Overlap = self.create_k_matrix(int_mat=self.Overlap)
    ...
    def map_sparse_indices(self, ):
        ...
        for IA in range(self.NA):
            indR = self.orb_per_at[IA] - 1
            orbA = self.orb_per_at[IA + 1] - self.orb_per_at[IA]
            # Layer_Matrix.dat can contain multiple entries for the same neighbor and being neighbor of itself
            neigh_list = []
            neigh_list.append(IA + 1)
            ...
            for IB in range(self.NB + 1):
                ...
                if IB == 0:
                    ...
                    if self.LM[IA, 4 + IB - 1] > 0 and self.LM[IA, 4 + IB - 1] not in neigh_list:
                        neigh = int(self.LM[IA, 4 + IB - 1])
                        indC = self.orb_per_at[neigh - 1] - 1
                        orbB = self.orb_per_at[neigh] - self.orb_per_at[neigh - 1]
            ...
            if (np.max(self.no_orb) == 1):
                ...
                indJ[indA:ind] = np.sort(indJ[indA:ind])
            indA = ind
        columns = indI[:ind].astype('int32')
        rows = indJ[:ind].astype('int32')
        sort_inds = np.lexsort((columns, rows))
        self.columns = columns[sort_inds]
        self.rows = rows[sort_inds]
        # self.columns = indI[:ind].astype('int32')
        # self.rows = indJ[:ind].astype('int32')
    ...
    def construct_coulomb_matrix(self, eps_r=1, eps0=8.854e-12, e=1.6022e-19, diag=False, orb_uniform=True, lattice_vectors=None, int_mat=None):
        """
        ...
        for key in int_mat.keys():
            if key == 'H_3':
                shift = np.array([0, 0, -1]) @ lattice_vectors
            elif key == 'H_4':
    ...
    def calculate_coulomb_matrix_elements(self, eps_r, eps0, e, shift=None, diag=False, orb_uniform=True, r0=0.3):
        """
        ...
        shift : ndarray, optional
            Shift vector for interactions with neighbouring unit cells. The default is None.
        ...
        if (shift != np.array([0, 0, 0])).any():
            # "Filling" the diagonal elements of the Coulomb matrix  
            diag = True
        ...
        for ia in range(NA):
            orbA = self.orb_per_at[ia + 1] - self.orb_per_at[ia]
            neigh_list = []
            neigh_list.append(ia)
            for ib in range(NB):
                neigh = int(LM[ia, 4 + ib] - 1)
                if neigh >= 0 and neigh not in neigh_list:  # or not equal to 0
                    neigh_list.append(neigh)
                    ...
                    Vact = factor / dist * np.tanh(dist/r0)
        ...
        for ia in range(NA):
            ...
            if diag:
                ...
                if orb_uniform:
                    ...
                    V_atomic[ia, 0, 0:orbA, 0:orbA] = Vact * (np.ones((orbA, orbA), dtype = np.cfloat))
                else:
                    V_atomic[ia, 0, 0:orbA, 0:orbA] = Vact * SF[0:orbA, 0:orbA]
            elif orbA > 1:
                if orb_uniform:
        ...
        return self.map_4D_to_sparse(V_atomic)

    def map_4D_to_sparse(self, V_atomic):
        """
        Parameters
        ----------
        ...
        """
        indI = np.zeros((self.NA * (self.NB + 1) * self.TB * self.TB, ), dtype=int)
        indJ = np.zeros((self.NA * (self.NB + 1) * self.TB * self.TB, ), dtype=int)
        NNZ = np.zeros((self.NA * (self.NB + 1) * self.TB * self.TB, ), dtype=complex)

        ind = 0

        for IA in range(self.NA):

            indR = self.orb_per_at[IA]
            orbA = self.orb_per_at[IA + 1] - self.orb_per_at[IA]
            ...
            for IB in range(self.NB + 1):
                ...
                if IB == 0:
                    ...
                    if self.LM[IA, 4 + IB - 1] > 0 and self.LM[IA, 4 + IB - 1] not in neigh_list:
                        ...
                        neigh_list.append(int(self.LM[IA, 4 + IB - 1]))

                        neigh = int(self.LM[IA, 4 + IB - 1] - 1)

                        indC = self.orb_per_at[neigh]
                        orbB = self.orb_per_at[neigh + 1] - self.orb_per_at[neigh]
        ...
        sparse_shape = np.max(self.orb_per_at) - self.orb_per_at[0]
    ...
    def scale_coulomb_matrix(self, factor):
        """
        Scales the Coulomb matrix with a factor.
        ...
        """
        for key in self.k_Coulomb_matrix.keys():
            self.k_Coulomb_matrix[key] *= factor