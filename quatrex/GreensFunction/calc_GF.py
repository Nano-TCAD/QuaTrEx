...
from utilss.matrix_creation import initialize_block_G, mat_assembly_fullG
...
def calc_GF(DH, E, GR, GL, GG, SigR, SigL, SigG, Efl, Efr, Temp):
    ...
    for IE in range(NE):
        ...
        rgf_GF(M, SigG[IE], SigL[IE], GR_3D_E[IE], GRnn1_3D_E[IE], GL_3D_E[IE], GLnn1_3D_E[IE], GG_3D_E[IE],
               GGnn1_3D_E[IE], fL[IE], fR[IE], DH.Bmin.copy(), DH.Bmax.copy())
        # we want to create a list of sparse matrices, one for each type of Green's function
        GR[IE] = factor[IE] * mat_assembly_fullG(
            GR_3D_E[IE], GRnn1_3D_E[IE], DH.Bmin.copy(), DH.Bmax.copy(), format='sparse', type='R')
        GL[IE] = factor[IE] * mat_assembly_fullG(
            GL_3D_E[IE], GLnn1_3D_E[IE], DH.Bmin.copy(), DH.Bmax.copy(), format='sparse', type='L')
        GG[IE] = factor[IE] * mat_assembly_fullG(
            GG_3D_E[IE], GGnn1_3D_E[IE], DH.Bmin.copy(), DH.Bmax.copy(), format='sparse', type='G')
