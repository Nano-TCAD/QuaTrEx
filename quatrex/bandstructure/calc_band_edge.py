...
def get_cv_band_edges_mpi_interpol(ECmin_DFT: float,
                               ...
                               side: str = 'left'):
    """
    This function calculates the conduction and valence band edge of the system.
    ...
    (min_ind, send_rank_1, send_rank_2) = get_send_ranks_interpol(
        EVmax_int, E, comm, rank, size, count, disp)
    send_sigmas_GWRGL_PHNR_to_root(SigmaR_GW_vec, SigmaL_GW_vec, SigmaG_GW_vec, SigmaR_PHN_vec,
                                   send_rank_1, send_rank_2, min_ind, rank, comm, disp, SigmaR_GW, SigmaL_GW, SigmaG_GW, SigmaR_PHN, rows, columns, nao)
    if rank == 0:
        Ek = calc_bandstructure_mpi_interpol(
        ...
        print("send rank 1 was: " + str(send_rank_1) + " send rank 2 was: " +
              str(send_rank_2) + " new band edge: " + str(EVmax))
    else:
        EVmax = np.empty(1, dtype=np.float64)
        comm.Bcast([EVmax, MPI.DOUBLE], root=0)
        EVmax = EVmax[0]
    # returning the band edges and the index of the band edge in the eigenvalue problem
...
def get_cband_edge_mpi_interpol_2_allblocks(EEdge,
                      E,
                      S,
                      H,
                      SigmaR_GW,
    ...
    # For this function, it is assumed that ind_ek_plus has been determined before and is the same for all blocks
    NB = Bmin.shape[0]
    CB_edge_int = EEdge[0, :]
    CB_edge_refined = np.zeros(NB, dtype = np.float64)

    for block_idx in range(NB):
        # First step: get a first estimate of the CB edge
        (min_ind, send_rank_1, send_rank_2) = get_send_ranks_interpol(CB_edge_int[block_idx], E, comm, rank, size, count, disp)
        send_sigmas_GWRGL_PHNR_to_root_2(SigmaR_GW_vec,send_rank_1, send_rank_2, min_ind, rank, comm, disp,  SigmaR_GW, nnz)
        SigmaR_GW_vec[:] = (SigmaR_GW_vec + SigmaR_GW_vec[:, ij2ji]) / 2
        if rank == 0:
            Ek = calc_bandstructure_mpi_interpol_2(E, S, H, CB_edge_int[block_idx], SigmaR_GW_vec, min_ind, Bmin, Bmax, 'device', mapdiag, mapupper, maplower, block_idx)
            ECmin_int = Ek[ind_ek_plus]
            # broadcasting the band edge (this is actually not necessary, but it is done for consistency), non-root nodes will not use it in get_send_ranks_interpol 
            comm.Bcast([ECmin_int, MPI.DOUBLE], root=0)
        else:   
        ...
        SigmaR_GW_vec[:] = (SigmaR_GW_vec + SigmaR_GW_vec[:, ij2ji]) / 2
        if rank == 0:
            Ek = calc_bandstructure_mpi_interpol_2(E, S, H, ECmin_int, SigmaR_GW_vec, min_ind, Bmin, Bmax, 'device', mapdiag, mapupper, maplower, block_idx)
            # ind_ek_plus = np.argmin(np.abs(Ek - ECmin_int))
            ECmin = Ek[ind_ek_plus]
            # broadcasting the band edge
            comm.Bcast([ECmin, MPI.DOUBLE], root=0)
        ...
        CB_edge_refined[block_idx] = ECmin

    # returning the band edge and the index of the band edge in the eigenvalue problem
    return CB_edge_refined

def get_vband_edge_mpi_interpol_2_allblocks(EEdge,
                      E,
                      S,
                      H,
                      SigmaR_GW,
    ...
    NB = Bmin.shape[0]
    VB_edge_int = EEdge[1, :]
    VB_edge_refined = np.zeros(NB, dtype = np.float64)

    for block_idx in range(NB):
        # First step: get a first estimate of the VB edge
        (min_ind, send_rank_1, send_rank_2) = get_send_ranks_interpol(VB_edge_int[block_idx], E, comm, rank, size, count, disp)
        send_sigmas_GWRGL_PHNR_to_root_2(SigmaR_GW_vec,send_rank_1, send_rank_2, min_ind, rank, comm, disp,  SigmaR_GW, nnz)
        SigmaR_GW_vec[:] = (SigmaR_GW_vec + SigmaR_GW_vec[:, ij2ji]) / 2
        if rank == 0:
            Ek = calc_bandstructure_mpi_interpol_2(E, S, H, VB_edge_int[block_idx], SigmaR_GW_vec, min_ind, Bmin, Bmax, 'device', mapdiag, mapupper, maplower, block_idx)
            EVmax_int = Ek[ind_ek_plus]
            # broadcasting the band edge (this is actually not necessary, but it is done for consistency), non-root nodes will not use it in get_send_ranks_interpol 
            comm.Bcast([EVmax_int, MPI.DOUBLE], root=0)
        else:   
            EVmax_int = np.empty(1, dtype=np.float64)
        ...
        SigmaR_GW_vec[:] = (SigmaR_GW_vec + SigmaR_GW_vec[:, ij2ji]) / 2
        if rank == 0:
            Ek = calc_bandstructure_mpi_interpol_2(E, S, H, EVmax_int, SigmaR_GW_vec, min_ind, Bmin, Bmax, 'device', mapdiag, mapupper, maplower, block_idx)
            # ind_ek_plus = np.argmin(np.abs(Ek - ECmin_int))
            EVmax = Ek[ind_ek_plus]
            # broadcasting the band edge
            comm.Bcast([EVmax, MPI.DOUBLE], root=0)
            print("send rank 1 was: " + str(send_rank_1) + " send rank 2 was: " + str(send_rank_2) + " new band edge: " + str(EVmax))
        else:   
            EVmax = np.empty(1, dtype=np.float64)
            comm.Bcast([EVmax, MPI.DOUBLE], root=0)
            EVmax = EVmax[0]

        VB_edge_refined[block_idx] = EVmax

    # returning the band edge and the index of the band edge in the eigenvalue problem
    return VB_edge_refined
...
if __name__ == '__main__':
    ...
    sr_h2g_full = np.loadtxt('/usr/scratch/mont-fort17/dleonard/CNT/python_testfiles/calc_band_edge_interpol/sr_gw.dat').view(complex)
    sl_h2g_full = np.loadtxt('/usr/scratch/mont-fort17/dleonard/CNT/python_testfiles/calc_band_edge_interpol/sl_gw.dat').view(complex)
    sg_h2g_full = np.loadtxt('/usr/scratch/mont-fort17/dleonard/CNT/python_testfiles/calc_band_edge_interpol/sg_gw.dat').view(complex)
