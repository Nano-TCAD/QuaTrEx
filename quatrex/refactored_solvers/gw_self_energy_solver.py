...
def compute_gw_self_energy_lesser_greater(
    ...
    Screened_interaction_greater_fourier = fft.fft(
        Screened_interaction_greater, n=2*number_of_energy_points)
    Screened_interaction_lesser_fourier = fft.fft(
        Screened_interaction_lesser, n=2*number_of_energy_points)
    ...
    Sigma_greater = scaling_factor * \
        fft.ifft(Sigma_greater_fourier)[:, :number_of_energy_points]
    Sigma_lesser = scaling_factor * \
        fft.ifft(Sigma_lesser_fourier)[:, :number_of_energy_points]
...
def compute_gw_self_energy_lesser_greater_correction(
    ...
    # fft
    Screened_interaction_greater_transposed_fourier = fft.fft(
        Screened_interaction_greater_transposed, n=2*number_of_energy_points)
    Screened_interaction_lesser_transposed_fourier = fft.fft(
        Screened_interaction_lesser_transposed, n=2*number_of_energy_points)

    # fft of energy reversed
    G_greater_reversed_fourier = fft.fft(
        np.flip(G_greater, axis=-1), n=2*number_of_energy_points)
    G_lesser_reversed_fourier = fft.fft(
        np.flip(G_lesser, axis=-1), n=2*number_of_energy_points)
    ...
    Sigma_greater_correction_fourier = G_greater_reversed_fourier *\
        (Screened_interaction_lesser_transposed_fourier -
         Screened_interaction_lesser_transposed[:, 0].reshape(-1, 1))
    Sigma_lesser_correction_fourier = G_lesser_reversed_fourier *\
        (Screened_interaction_greater_transposed_fourier -
         Screened_interaction_greater_transposed[:, 0].reshape(-1, 1))

    Sigma_greater_correction2 = scaling_factor *np.flip(fft.ifft(G_greater_reversed_fourier*\
        (Screened_interaction_lesser_transposed_fourier))
                [:, :number_of_energy_points], axis=-1)

    Sigma_greater_correction2 += -scaling_factor*\
                    G_greater*\
                    Screened_interaction_lesser_transposed[:, 0].reshape(-1,1)


    # ifft, multiply with scaling factor and cutoff to original size
    Sigma_greater_correction = scaling_factor * \
        np.flip(fft.ifft(Sigma_greater_correction_fourier)
                [:, :number_of_energy_points], axis=-1)
    Sigma_lesser_correction = scaling_factor * \
        np.flip(fft.ifft(Sigma_lesser_correction_fourier)
                [:, :number_of_energy_points], axis=-1)
...
def compute_gw_self_energy_retarded(
    ...
    energy_array = np.linspace(
        0, (number_of_energy_points-1)*delta_energy, number_of_energy_points)
    ...
    Sigma_density = -scaling_factor_abs * \
        (G_lesser_density*Coulomb_matrix).reshape(-1, 1)
    Sigma_retarded_correction = np.repeat(
        Sigma_density, number_of_energy_points, axis=-1)
    ...
    rSigma_retarded = 2*scaling_factor * \
        fft.ifft(rSigma_retarded_fourier)[:, number_of_energy_points-1:-1]
