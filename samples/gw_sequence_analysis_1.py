...
if __name__ == "__main__":
    ...
    for iteration in range(start_iter+1, stop_iter):
        for idx_e in range(start_energy, stop_energy):
            ...
            SigmaR_GW = np.load(os.path.join(self_energy_path, "sigma_R_{}.npy".format(iteration-1)))
            # Reading left boundary self-energy
            SigmaR_Bl = np.load(os.path.join(self_energy_path, "sigma_RBL_{}.npy".format(iteration-1)))
            # Reading right boundary self-energy
            SigmaR_Br = np.load(os.path.join(self_energy_path, "sigma_RBR_{}.npy".format(iteration-1)))
            ...
            G_im1 = np.linalg.inv(matrix.toarray())
            ...
            cond_A = np.linalg.cond(A_i)
            
            A_i_conditionned = G_im1 @ A_i
            
            cond_Aim1xAi = np.linalg.cond(A_i_conditionned)
            ...
            if iteration == 1 and idx_e == 0:
                print("Matrix size: " + str(A_i.shape[0]) + "x" + str(A_i.shape[1]))
    ...
    import csv
    with open('conditionning_from_precedent_1to16_iter.csv', 'w', newline='') as file:
