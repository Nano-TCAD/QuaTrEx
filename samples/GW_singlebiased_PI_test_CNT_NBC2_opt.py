...
if __name__ == "__main__":
    ...
    no_orb = np.array([1,1])
    Vappl = 0.2
    energy = np.linspace(-10, 5, 376, endpoint=True, dtype=float)  # Energy Vector
    Idx_e = np.arange(energy.shape[0])  # Energy Index Vector
    hamiltonian_obj = OMENHamClass.Hamiltonian(args.file_hm, no_orb, Vappl=Vappl, rank=rank, layer_matrix = '/Layer_Matrix64.dat')
    ...
    for iter_num in range(max_iter):
        ...
        if args.pool:
            _, _, gl_diag, gl_upper, gg_diag, gg_upper, sigRBl, sigRBr = calc_GF_pool.calc_GF_pool_mpi(
                hamiltonian_obj,
                energy_loc,
                sr_h2g_vec,
                sl_h2g_vec,
        ...
        if args.type in ("gpu"):
            pg_g2p, pl_g2p, pr_g2p = g2p_gpu.g2p_fft_mpi_gpu(pre_factor, gg_g2p, gl_g2p, np.zeros_like(gl_g2p), gl_transposed_g2p)
        elif args.type in ("cpu"):
            pg_g2p, pl_g2p = g2p_cpu.g2p_fft_mpi_cpu_inlined_nopr(pre_factor, gg_g2p, gl_g2p,
                                                                     gl_transposed_g2p)
        ...
        if args.type in ("gpu"):
            sg_gw2s, sl_gw2s, sr_gw2s = gw2s_gpu.gw2s_fft_mpi_gpu_3part_sr(-pre_factor / 2, gg_g2p, gl_g2p, np.zeros_like(gl_g2p),
                                                                           wg_gw2s, wl_gw2s, wl_gw2s,
    ...
    if rank == 0:
        ...
        print(f"Green's Function differences to Gold Solution g/l:  {diff_gg:.4f}, {diff_gl:.4f}")
        print(f"Polarization differences to Gold Solution g/l:  {diff_pg:.4f}, {diff_pl:.4f}")
        ...
        assert np.allclose(gg_gold, gg_mpi, atol=1e-4, rtol=1e-4)
        assert np.allclose(gl_gold, gl_mpi, atol=1e-4, rtol=1e-4)
        #assert np.allclose(gr_gold, gr_mpi, atol=1e-4, rtol=1e-4)
        assert np.allclose(pg_gold, pg_mpi, atol=1e-4, rtol=1e4)
        assert np.allclose(pl_gold, pl_mpi, atol=1e-4, rtol=1e-4)
        ...
        assert np.allclose(wl_gold, wl_mpi, atol=1e-4, rtol=1e-4)
        #assert np.allclose(wr_gold, wr_mpi, atol=1e-4, rtol=1e-4)
        assert np.allclose(sg_gold, sg_mpi, atol=1e-4, rtol=1e-4)
        assert np.allclose(sl_gold, sl_mpi, atol=1e-4, rtol=1e-4)
        assert np.allclose(sr_gold, sr_mpi, atol=1e-4, rtol=1e-4)
