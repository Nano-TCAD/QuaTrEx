...
if __name__ == "__main__":
    ...
    data_shape = np.array([rows.shape[0], energy.shape[0]*nkpts], dtype=np.int32)
    ...
    for iter_num in range(max_iter):
        ...
        if args.type in ("cpu"):
            ...
            sg_gw2s, sl_gw2s, sr_gw2s = gw2s_gpu.gw2s_fft_mpi_gpu_PI_sr_batched(-pre_factor / 2, gg_g2p, gl_g2p,
                                                                           wg_gw2s, wl_gw2s,
                                                                           wg_transposed_gw2s, wl_transposed_gw2s, vh1d, energy, rank, disp, count,  batch_size = 880)
        else:
            raise ValueError("Argument error, input type not possible")
        ...
        if (iter_num % 25) == 0 and iter_num > 1:
            sl_rgf_dev = cp.asarray(sl_h2g)
            sg_rgf_dev = cp.asarray(sg_h2g)
            sr_rgf_dev = cp.asarray(sr_h2g)
            sl_phn_dev = cp.asarray(sl_phn)
            ...
            if rank == 0:
                np.savetxt(SE_path + 'Vg_' + str(iter_num) + '.dat', np.array([hamiltonian_obj.poisson_Vg]))
                np.savetxt(SE_path + 'Vatom_' + str(iter_num) + '.dat', hamiltonian_obj.Vatom)
                np.savetxt(SE_path + 'VPoiss_' + str(iter_num) + '.dat', hamiltonian_obj.Vpoiss)
                np.savetxt(SE_path + 'ECmin.dat', ECmin_vec.T)
                np.savetxt(SE_path + 'ind_ek_cb_l_' + str(iter_num) + '.dat', np.array([ind_ek_cb_l]))
                np.savetxt(SE_path + 'ind_ek_cb_r_' + str(iter_num) + '.dat', np.array([ind_ek_cb_r]))
