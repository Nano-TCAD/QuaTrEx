...
from quatrex.OMEN_structure_matrices.OMENHamClass import Hamiltonian

from quatrex.GreensFunction.calc_GF import calc_GF
from quatrex.GreensFunction.calc_GF_pool import calc_GF_pool
...
from quatrex.utils.change_format import (map_block2sparse, block2sparse_energy, block2sparse_energy_alt,
                                         map_block2sparse_alt, sparse2vecsparse)
from quatrex.GW.screenedinteraction.kernel.calc_W import calc_W
from quatrex.GW.screenedinteraction.kernel.calc_W_pool import calc_W_pool
...
energy, rows, columns, gg_gold, gl_gold, gr_gold = read_solution.load_x(
    "/usr/scratch/mont-fort17/dleonard/CNT/data_GPWS_04.mat", "g")
energy, rows_p, columns_p, pg_gold, pl_gold, pr_gold = read_solution.load_x(
    "/usr/scratch/mont-fort17/dleonard/CNT/data_GPWS_04.mat", "p")
energy, rows_w, columns_w, wg_gold, wl_gold, wr_gold = read_solution.load_x(
    "/usr/scratch/mont-fort17/dleonard/CNT/data_GPWS_04.mat", "w")
energy, rows_s, columns_s, sg_gold, sl_gold, sr_gold = read_solution.load_x(
    "/usr/scratch/mont-fort17/dleonard/CNT/data_GPWS_04.mat", "s")
...
map_diag, map_upper, map_lower = map_block2sparse_alt(rows, columns, DH.Bmax - 1, DH.Bmin - 1)
...
while (iteration <= max_iteration) and (crit > max_error):
    ...
    GLn1n_reduced = np.array(
        list(
            map(matrix_creation.negative_hermitian_transpose,
                GLnn1_3D_E.reshape(((NB - 1) * (ne_f - ne_s), Bsize, Bsize)))))
    GLn1n_3D_E = GLn1n_reduced.reshape((ne_f - ne_s, NB - 1, Bsize, Bsize))

    GGn1n_reduced = np.array(
        list(
            map(matrix_creation.negative_hermitian_transpose,
                GGnn1_3D_E.reshape(((NB - 1) * (ne_f - ne_s), Bsize, Bsize)))))
    GGn1n_3D_E = GGn1n_reduced.reshape((ne_f - ne_s, NB - 1, Bsize, Bsize))

    gr_computed = block2sparse_energy_alt(map_diag, map_upper, map_lower, GR_3D_E, GRnn1_3D_E, GRn1n_3D_E, rows.size,
                                          ne_f - ne_s)

    gl_computed = block2sparse_energy_alt(map_diag, map_upper, map_lower, GL_3D_E, GLnn1_3D_E, GLn1n_3D_E, rows.size,
                                          ne_f - ne_s)

    gg_computed = block2sparse_energy_alt(map_diag, map_upper, map_lower, GG_3D_E, GGnn1_3D_E, GGn1n_3D_E, rows.size,
                                          ne_f - ne_s)
    ...
    pg_computed, pl_computed, pr_computed = g2p_cpu.g2p_fft_cpu_inlined(pre_factor, ij2ji, gg_computed, gl_computed,
                                                                        gr_computed)
    ...
    wg_computed = change_format.block2sparse_energy_alt(map_diag_W, map_upper_W, map_lower_W, WG_3D_E, WGnn1_3D_E,
                                                        WGn1n_3D_E, rows.size, ne_f - ne_s)
    wl_computed = change_format.block2sparse_energy_alt(map_diag_W, map_upper_W, map_lower_W, WL_3D_E, WLnn1_3D_E,
                                                        WLn1n_3D_E, rows.size, ne_f - ne_s)
    wr_computed = change_format.block2sparse_energy_alt(map_diag_W, map_upper_W, map_lower_W, WR_3D_E, WRnn1_3D_E,
    ...
    sg_cpu, sl_cpu, sr_cpu = gw2s_cpu.gw2s_fft_cpu(-pre_factor / 2, ij2ji, gg_computed, gl_computed, gr_computed,
...
print("Transforming polarization into vector of sparse matrices took: " + "%.2f" % (toc_trafo_p2w - tic_trafo_p2w) +
      " [s]")
