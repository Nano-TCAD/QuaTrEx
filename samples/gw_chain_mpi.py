...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--pool", default="True", choices=["True", "False"], required=False)
    ...
    sg_h2g = np.empty((count[1, rank], data_shape[0]), dtype=np.complex128, order="C")
    sl_h2g = np.empty((count[1, rank], data_shape[0]), dtype=np.complex128, order="C")
    sr_h2g = np.empty((count[1, rank], data_shape[0]), dtype=np.complex128, order="C")
    if args.type in ("gpu"):
        # start gpu streams
    ...
    if args.pool:
        gr_diag, gr_upper, gl_diag, gl_upper, gg_diag, gg_upper = calc_GF_pool.calc_GF_pool_mpi(
            hamiltionian_obj, energy_loc, sr_h2g_vec, sl_h2g_vec, sg_h2g_vec, energy_fl, energy_fr, temp,
    ...
    if args.type in ("gpu"):
        g2p_gpu.g2p_fft_mpi_gpu_streams(pre_factor, gg_g2p, gl_g2p, gr_g2p, gl_transposed_g2p, pg_g2p, pl_g2p, pr_g2p,
                                        streams)
    elif args.type in ("cpu"):
        pg_g2p, pl_g2p, pr_g2p = g2p_cpu.g2p_fft_mpi_cpu_inlined(pre_factor, gg_g2p, gl_g2p, gr_g2p, gl_transposed_g2p)
    else:
    ...
    if args.pool in ("True"):
        wg_diag, wg_upper, wl_diag, wl_upper, wr_diag, wr_upper, _, _ = p2w_cpu.p2w_pool_mpi_cpu(
            hamiltionian_obj, energy_loc, pg_p2w_vec, pl_p2w_vec, pr_p2w_vec, vh,
            dosw[disp[1, rank]:disp[1, rank] + count[1, rank]], factor_w_loc, w_mkl_threads, w_worker_threads)
    else:
        wg_diag, wg_upper, wl_diag, wl_upper, wr_diag, wr_upper, _, _ = p2w_cpu.p2w_mpi_cpu(
            hamiltionian_obj, energy_loc, pg_p2w_vec, pl_p2w_vec, pr_p2w_vec, vh,
            dosw[disp[1, rank]:disp[1, rank] + count[1, rank]], factor_w_loc, w_mkl_threads)
    # timing compute
    times_compute[2] += time.perf_counter()
    ...
    if args.type in ("gpu"):
        sg_gw2s, sl_gw2s, sr_gw2s = gw2s_gpu.gw2s_fft_mpi_gpu(-pre_factor / 2, gg_g2p, gl_g2p, gr_g2p, wg_gw2s, wl_gw2s,
                                                              wr_gw2s, wg_transposed_gw2s, wl_transposed_gw2s)
    elif args.type in ("cpu"):
        sg_gw2s, sl_gw2s, sr_gw2s = gw2s_cpu.gw2s_fft_mpi_cpu(-pre_factor / 2, gg_g2p, gl_g2p, gr_g2p, wg_gw2s, wl_gw2s,
                                                              wr_gw2s, wg_transposed_gw2s, wl_transposed_gw2s)
    else:
        raise ValueError("Argument error, input type not possible")
